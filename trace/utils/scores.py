...
from trace.utils.decorators import ignore_nodes
...
def hierarchical_set_score(frame1, frame2):
    ...
    # in general, score_tmp_1 > score > score_tmp_2

    # score_tmp_1 = 0.5 * act_score[0] + \
    #               0.1 * slot_score[0] + \
    #               0.1 * value_score[0] + \
    #               0.1 * act_slot_score[0] + \
    #               0.1 * slot_value_score[0] + \
    #               0.1 * act_slot_value_score[0]

    # grams1 = act_nodes1 + slot_nodes1 + value_nodes1 + act_slot_edges1 + slot_value_edges1 + act_slot_value_paths1
    # grams2 = act_nodes2 + slot_nodes2 + value_nodes2 + act_slot_edges2 + slot_value_edges2 + act_slot_value_paths2
    # score_tmp_2 = jaccard_dis_sim(x=grams1, y=grams2)[0]
    return score
